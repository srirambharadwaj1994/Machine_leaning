...
def manhattandistance(instance1, instance2):
    """ we are returning the sum of all absolute values which is obtained by subtracting instance1 and instance2"""
    return sum(abs(instance1 - instance2))


def minkowskidistance(instance1, instance2 , p=2):
    """minkkowski works based on the value of p if we assign p=1 it will calculate manhattan and if p=2 euclidian"""
...
def getNeighbors(cancer_training, testInstance, k):
    ...
    #calculating the distance between the neighbours using the Distance (selecting different distance-methods)
...
def getAccuracy(cancer_test, predictions):
    """#getting the accuracy count based on the prediction value which is calculated inside the main function"""
    correct = 0
